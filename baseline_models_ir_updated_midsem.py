# -*- coding: utf-8 -*-
"""Baseline_Models_IR_updated_midsem.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1A3BOXdE3Wnti4I-UmhK3FwZKill9w9ZZ
"""
...
def Text(event,root,model,count_vector):
  def get_result(event,root,input,model):
    val=input.get()
    print(val)
    val=pd.Series(val)
    print(type(val))
    train_c = count_vector.transform(val)
    print('Vocabulary:', count_vector.vocabulary_)
    train_c=train_c.toarray()
    print(train_c.shape)
    predictions_ann = model.predict(train_c)
    predictions_ann = [True if x >= 0.5 else False for x in predictions_ann]
    print(predictions_ann)
    str="Output is "+f'{predictions_ann[0]}'
    label1 = Label(root, text=str, font=('Helvetica bold', 20))
    label1.place(x=500, y=750)
    pass
  ...
  for i in range(1, len(children)):
    children[i].destroy()
  text = StringVar()
  label = Label(root, text="Enter Review that you want to check", font=('Helvetica bold', 15))
  label.place(x=200, y=600)
  input = Entry(root, textvariable=text)
  input.place(x=700, y=600)
  but = Button(root, text='Enter')
  but.place(x=1100, y=600)
  but.bind('<Button-1>', lambda event:get_result(event,root,input,model))

root=Tk()
root.geometry('1500x1500')
root.configure(bg='#B2FFFF')
'''
...
'''
label1=Label(root,text='Welcome to Fake Review Detection System',font=('Helvetica bold',20))
label1.place(x=260,y=10)
button1=Button(root,text='Enter Text',width=10,height=5)
button1.place(x=500,y=500)
button1.bind('<Button-1>',lambda event:Text(event,root,model_ann,count_vectorizer))
'''
